...
"""
    ...
    @Test
    fun testResolveStdKlutterExt() {
        val savedErr = System.err
        try {
            System.setErr(PrintStream(NullOutputStream()))
            Assert.assertNull(compileScript("args-klutter-ext.kts", StandardArgsScriptTemplateWithLocalResolving::class))
        }
        finally {
            System.setErr(savedErr)
        }

        val scriptClass = compileScript("args-klutter-ext.kts", StandardArgsScriptTemplateWithMavenResolving::class)
        if (scriptClass == null) {
            System.err.println(contextClasspath(KOTLIN_JAVA_RUNTIME_JAR, Thread.currentThread().contextClassLoader)?.joinToString())
        }
        Assert.assertNotNull(scriptClass)
